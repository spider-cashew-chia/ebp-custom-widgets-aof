(function () {
  ...
  function init() {
    try {
      ...
      const homeTextWrapper = document.querySelector('.ebp-header-1-home-text-wrapper');
      ...
      if (homeTextWrapper || homeBgImage) {
        ...
        function checkAnimationComplete() {
          ...
          const textFields = [homeText1, homeText2, homeText3, homeText4].filter(el => el !== null);
          const completedFields = [text1Complete, text2Complete, text3Complete, text4Complete].filter(complete => complete === true);
          
          // Proceed when all existing text fields have completed their animations
          const shouldProceed = textFields.length > 0 ? completedFields.length === textFields.length : true;
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  ...
                  if (animationContainer) {
                    animationContainer.style.transition = 'opacity 0.6s ease-out';
                    animationContainer.style.opacity = '0';
                    animationContainer.style.visibility = 'hidden';
                    animationContainer.style.pointerEvents = 'none';
                  }
                }, 4000); // Match the mask expansion duration
        ...
        function cycleHomeTextCharacters(element) {
          ...
          element.cycleInterval = setInterval(() => {
            ...
            for (let i = 0; i < originalText.length; i++) {
              // Calculate how many characters should be revealed
              const revealProgress = currentStep / steps;
              const shouldReveal = i < Math.floor(originalText.length * revealProgress);
    ...
    try {
      const menuBgForImages = document.querySelector('.ebp-header-1-menu-toggle--menu-bg');
      ...
      function showFeaturedImage(link, imageUrl) {
        ...
        if (!imageUrl) {
          ...
          const existingImages = menuBgForImages.querySelectorAll('.ebp-header-1-menu-featured-image');
    ...
    try {
      ...
      const menuBg = document.querySelector('.ebp-header-1-menu-toggle--menu-bg');
